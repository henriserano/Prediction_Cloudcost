...
def list_(
    ...
    return ConversationListResponse(
        conversations=[ConversationSummary(**c) for c in items],
    )
...
def get_one(
    ...
    return ConversationDetailResponse(
        thread_id=thread_id,
        messages=[ConversationMessage(**m) for m in load_messages(user_id, thread_id)],
    )
...
@router.delete(
    ...
    summary="Delete one conversation from history",
)
def delete(
    thread_id: str,
    user_id: Annotated[str, Depends(require_current_user_id)],
    ...
    return ConversationDeleteResponse(thread_id=thread_id, deleted=True)
